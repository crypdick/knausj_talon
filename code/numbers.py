...
def parse_number(list_: List[str]) -> str:
    """
    Parses a list of words into a number/digit string.
    """
    list_ = list(scan_small_numbers(list_))
    for scale in scales:
        list_ = parse_scale(scale, list_)
    return "".join(str(n) for n in list_)


def scan_small_numbers(list_: List[str]) -> Iterator[Union[str, int]]:
    """
    Takes a list of number words, yields a generator of mixed numbers &
    strings. Translates small number terms (<100) into corresponding numbers.
    Drops all occurrences of "and". Smashes digits onto tens words, eg.

    ["twenty", "one"] -> [21]. But note that "ten" and "zero" are excluded, ie:

    ["ten", "three"] -> [10, 3]   ["fifty", "zero"] -> [50, 0] Does
    nothing to scale words ("hundred", "thousand", "million", etc).
    """
    # reversed so that repeated pop() visits in left-to-right order
    list_ = [x for x in reversed(list_) if x != "and"]
    while list_:
        n = list_.pop()
        # fuse tens onto digits, eg. "twenty", "one" -> 21
        if n in tens_map and n != "ten" and list_ and digits_map.get(list_[-1], 0) != 0:
            d = list_.pop()
...
def parse_scale(scale: str, list_: List[Union[str, int]]) -> List[Union[str, int]]:
    ...
    left, *splits = split_list(scale, list_)
    for right in splits:
...
def split_list(value, list_: list) -> Iterator:
    """
